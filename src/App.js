...
const sumPay= d  => Object.values(d.payments||{}).reduce((a,b)=>a+b,0);

/* ═══════════════════════════════════════════════
   STORAGE — window.storage (survives refresh)
═══════════════════════════════════════════════ */
const STORE_KEY = "sft_v6";
...
function migrateToV6(d) {
  ...
  Object.keys(d.months||{}).sort().forEach(mk => {
    ...
    (md.debts||[]).forEach(debt => {
      const key = debt.name?.toLowerCase();
      if (!seen[key]) {
        seen[key] = true;
        globalDebts.push({
          id: debt.id||uid(), name: debt.name||"", totalDebt: debt.totalDebt||0,
          dueDate: debt.dueDate||"", completed: debt.completed||false, notes: debt.notes||"",
          payments: (debt.paid>0) ? {[mk]: debt.paid} : {}
        });
      } else {
        const gd = globalDebts.find(g=>g.name?.toLowerCase()===key);
        if (gd && debt.paid>0) gd.payments[mk] = (gd.payments[mk]||0)+debt.paid;
      }
    });
...
function blankMonth(tmpl) {
  const t = tmpl||defaultTemplates();
  return {
    income:   t.income.map(x=>({ id:uid(), source:x.source, budgeted:x.budgeted, actual:0 })),
    expenses: t.expenses.map(x=>({ id:uid(), name:x.name, budgeted:x.budgeted, actual:0, paid:false, category:x.category||"Essential", notes:"", linkedDebtId:null })),
  };
}
...
function calcMonth(md, globalDebts, monthKey) {
  ...
  const dpaid = (globalDebts||[]).reduce((s,d)=>s+((d.payments||{})[monthKey]||0),0);
  return { inc, exp, dpaid, sav: inc-exp-dpaid };
}

function cumSavings(months, globalDebts) {
  return Object.keys(months).sort().reduce((sum,mk)=>{
    const { sav } = calcMonth(months[mk], globalDebts, mk);
    return sum + Math.max(0, sav);
  }, 0);
}

function buildAlerts(md, globalDebts, monthKey) {
  const { inc, exp } = calcMonth(md, globalDebts, monthKey);
...
/* ═══════════════════════════════════════════════
   EXPORT
═══════════════════════════════════════════════ */
function exportCSV(monthKey, md, globalDebts) {
  ...
  csv+="\nEXPENSES\nName,Category,Budgeted,Actual,Paid,Notes,Linked Debt\n";
  (md.expenses||[]).forEach(r=>{
    const d=(globalDebts||[]).find(d=>d.id===r.linkedDebtId);
    csv+=`"${r.name}","${r.category||""}",${r.budgeted||0},${r.actual||0},${r.paid?"Yes":"No"},"${r.notes||""}","${d?.name||""}"\n`;
  });
  csv+="\nDEBTS (Global)\nName,Total Debt,Total Paid,This Month,Remaining,Due,Status\n";
  (globalDebts||[]).forEach(r=>{
    const tp=sumPay(r), rem=Math.max(0,(r.totalDebt||0)-tp), thisMo=(r.payments||{})[monthKey]||0;
    csv+=`"${r.name}",${r.totalDebt||0},${tp},${thisMo},${rem},"${r.dueDate||""}","${r.completed?"Done":"Active"}"\n`;
  });
...
function LoadingScreen() {
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#07090e,#0b0f17)",
      display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      <div style={{width:40,height:40,borderRadius:10,background:"linear-gradient(135deg,#06b6d4,#3b82f6)",
        display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,color:"#fff",fontSize:18}}>S</div>
      <div style={{color:"#4a90d9",fontSize:13}}>Loading your data…</div>
      <div style={{width:120,height:3,background:"rgba(255,255,255,0.06)",borderRadius:99,overflow:"hidden"}}>
        <div style={{height:"100%",background:"linear-gradient(90deg,#06b6d4,#3b82f6)",borderRadius:99,
          animation:"ldpulse 1.2s ease-in-out infinite"}}/>
      </div>
      <style>{`@keyframes ldpulse{0%{width:0%}50%{width:80%}100%{width:100%}}`}</style>
    </div>
  );
}
...
function NotesInput({ value, onChange }) {
  const [f,sf]=useState(false);
  const [open,so]=useState(!!value);
  if(!open) return (
    <button onClick={()=>so(true)}
      style={{background:"none",border:"none",color:value?"#67e8f9":"#2a3a4a",cursor:"pointer",
        fontSize:11,padding:"2px 6px",borderRadius:5,fontFamily:"inherit",transition:"color .15s"}}
      title={value||"Add note"}>{value?"📝":"＋note"}</button>
  );
  return <textarea value={value||""} onChange={e=>onChange(e.target.value)}
    onFocus={()=>sf(true)} onBlur={()=>sf(false)} placeholder="Notes..." rows={2}
    style={{background:"rgba(255,255,255,0.05)",border:`1px solid ${f?"rgba(6,182,212,0.4)":"rgba(255,255,255,0.1)"}`,
      borderRadius:7,padding:"5px 8px",color:"#c8d3e0",fontSize:11,outline:"none",
      width:"100%",fontFamily:"inherit",resize:"vertical",boxSizing:"border-box",transition:"all .15s"}}/>;
}

function DateInput({ value, onChange }) {
  return <input type="date" value={value} onChange={e=>onChange(e.target.value)}
    style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",
      borderRadius:8,padding:"6px 8px",color:"#8899aa",fontSize:12,width:"100%",outline:"none",
      colorScheme:"dark",fontFamily:"inherit",boxSizing:"border-box"}}/>;
}

function CatSelect({ value, onChange }) {
  return <select value={value||"Essential"} onChange={e=>onChange(e.target.value)}
    style={{background:"transparent",border:"none",color:CAT_CLR[value]||"#94a3b8",fontSize:11,
...
function DebtSelect({ value, onChange, globalDebts }) {
  ...
  return (
    <select value={value||""} onChange={e=>onChange(e.target.value||null)}
      title="Link to debt (auto-updates when paid)"
      style={{background:"transparent",border:"none",fontSize:10,cursor:"pointer",
        fontFamily:"inherit",outline:"none",maxWidth:90,
        color:value?"#818cf8":"#3d5166"}}>
      <option value="" style={{background:"#0d1117",color:"#3d5166"}}>🔗 Link debt</option>
      {active.map(d=><option key={d.id} value={d.id} style={{background:"#0d1117",color:"#c8d3e0"}}>
        {d.name.slice(0,18)}
      </option>)}
    </select>
  );
...
function Btn({ onClick, children, color="#06b6d4", variant="outline", size="md", disabled=false, title }) {
  ...
  return <button onClick={onClick} disabled={disabled} title={title}
    ...
    style={{display:"inline-flex",alignItems:"center",gap:5,padding:pad,borderRadius:8,fontSize:fs,
      fontWeight:700,cursor:disabled?"not-allowed":"pointer",fontFamily:"inherit",
      opacity:disabled?.5:1,transition:"all .18s",whiteSpace:"nowrap",...s}}>{children}</button>;
}
...
function AddBtn({ onClick, label, color="#06b6d4" }) {
  ...
  return <button onClick={onClick} onMouseEnter={()=>sh(true)} onMouseLeave={()=>sh(false)}
    style={{display:"inline-flex",alignItems:"center",gap:5,padding:"6px 13px",borderRadius:8,
      border:`1px solid ${h?color+"88":color+"33"}`,background:h?color+"20":color+"0d",color,
      fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all .18s",whiteSpace:"nowrap"}}>
...
function DelBtn({ onClick }) {
  const [h,sh]=useState(false);
  return <button onClick={onClick} onMouseEnter={()=>sh(true)} onMouseLeave={()=>sh(false)} title="Delete"
...
function Badge({ bg, fg, children }) {
  return <span style={{display:"inline-block",padding:"2px 8px",borderRadius:99,fontSize:10,
    fontWeight:700,background:bg,color:fg,whiteSpace:"nowrap"}}>{children}</span>;
}

function Card({ children, style={}, className="" }) {
  return <div className={className} style={{background:"rgba(255,255,255,0.027)",
    border:"1px solid rgba(255,255,255,0.065)",borderRadius:16,padding:"18px 16px",
    marginBottom:16,...style}}>{children}</div>;
}

function FadeRow({ children, visible }) {
  const [mounted,sm]=useState(true);
  const [show,ss]=useState(true);
  useEffect(()=>{
    if(visible){sm(true);const t=setTimeout(()=>ss(true),10);return()=>clearTimeout(t);}
    else{ss(false);const t=setTimeout(()=>sm(false),280);return()=>clearTimeout(t);}
  },[visible]);
  if(!mounted) return null;
  return <tr style={{opacity:show?1:0,transform:show?"none":"translateY(-6px)",
    transition:"opacity .28s,transform .28s"}}>{children}</tr>;
}

function ProgBar({ value, total, color="#818cf8", height=6 }) {
  return <div style={{height,background:"rgba(255,255,255,0.055)",borderRadius:99,overflow:"hidden"}}>
    <div style={{height:"100%",borderRadius:99,width:`${pct(value,total)}%`,
...
function SearchBar({ value, onChange, placeholder }) {
  const [f,sf]=useState(false);
  return <div style={{position:"relative",flex:"1 1 180px",minWidth:0}}>
    ...
    {value&&<button onClick={()=>onChange("")} style={{position:"absolute",right:8,top:"50%",
      transform:"translateY(-50%)",background:"none",border:"none",color:"#3d5166",
...
function Confirm({ title, detail, confirmLabel="Delete", onOk, onCancel }) {
  return <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,0.85)",
    display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={onCancel}>
    <div style={{background:"#0d1117",border:"1px solid rgba(255,255,255,0.12)",borderRadius:16,
      padding:"28px 24px",maxWidth:420,width:"100%",boxShadow:"0 24px 60px rgba(0,0,0,0.8)"}}
...
function MCard({ label, value, sub, color="#94a3b8", isHero, danger }) {
  ...
  if(isHero) return (
    <div onMouseEnter={()=>sh(true)} onMouseLeave={()=>sh(false)}
      style={{background:"linear-gradient(135deg,rgba(6,182,212,0.14),rgba(59,130,246,0.09))",
        border:"1px solid rgba(6,182,212,0.3)",borderRadius:14,padding:"16px 18px",flex:"1 1 150px",
        minWidth:0,transform:h?"translateY(-3px)":"none",transition:"transform .2s",overflow:"hidden"}}>
  ...
  return (
    <div onMouseEnter={()=>sh(true)} onMouseLeave={()=>sh(false)}
      style={{background:"rgba(255,255,255,0.027)",border:`1px solid ${color}18`,borderRadius:14,
        padding:"16px 18px",flex:"1 1 150px",minWidth:0,transform:h?"translateY(-2px)":"none",transition:"transform .2s"}}>
...
function DebtCard({ row, visible }) {
  ...
  const rem  = Math.max(0,(row.totalDebt||0)-paid);
  const done = row.completed||rem===0;
  const p    = pct(paid,row.totalDebt||1);
  ...
  return (
    <div style={{background:done?"rgba(5,150,105,0.07)":"rgba(255,255,255,0.025)",
      ...
      opacity:visible?1:0,transform:visible?"none":"translateY(10px)",transition:"opacity .3s,transform .3s"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,gap:8}}>
        ...
        {done?<Badge bg="rgba(5,150,105,0.18)" fg="#34d399">DONE</Badge>
      ...
      <div style={{marginBottom:4}}>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:10,color:"#3d5166",marginBottom:4}}>
          ...
          <span>{fmt(paid)} / {fmt(row.totalDebt||0)}</span>
        </div>
        <ProgBar value={paid} total={row.totalDebt||0} color={col}/>
      </div>
      ...
      {open&&(
        <div onClick={e=>e.stopPropagation()} style={{position:"absolute",top:42,left:38,zIndex:300,
          ...
          {[...allKeys].sort().map(k=>{
            ...
            return <div key={k} onClick={()=>{onChange(ky,km);so(false);}}
...
function HistoryMgr({ store, onUpdate, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.85)",
      display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"20px 16px",overflowY:"auto"}} onClick={onClose}>
      <div style={{background:"#0d1117",border:"1px solid rgba(255,255,255,0.1)",borderRadius:18,
        padding:"24px 20px",maxWidth:640,width:"100%",marginTop:40}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:8}}>
          <div>
            <div style={{fontSize:16,fontWeight:800,color:"#eef2f8"}}>History Management</div>
            <div style={{fontSize:11,color:"#3d5166",marginTop:2}}>Delete or reset saved months</div>
          </div>
        ...
        {sorted.length===0&&<div style={{color:"#3d5166",textAlign:"center",padding:40,fontSize:13}}>No history yet.</div>}
        {sorted.map(key=>{
          const[y,m]=k2ym(key);
          const{inc,exp,sav}=calcMonth(months[key],globalDebts,key);
          return (
            <div key={key} style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(255,255,255,0.06)",
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
                ...
                <Btn onClick={()=>sc({t:"month",key})} variant="danger" size="sm">Delete</Btn>
              </div>
        ...
        {cfm?.t==="all"&&<Confirm title="Clear All History?" detail="Every month will be permanently deleted."
        ...
        {cfm?.t==="sec"&&<Confirm title={`Reset ${cfm.sec}?`} detail="This section will be cleared."
...
function TemplateMgr({ templates, currentMonth, onSave, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.85)",
      display:"flex",alignItems:"flex-start",justifyContent:"center",padding:"20px 16px",overflowY:"auto"}} onClick={onClose}>
      <div style={{background:"#0d1117",border:"1px solid rgba(255,255,255,0.1)",borderRadius:18,
        padding:"24px 20px",maxWidth:660,width:"100%",marginTop:40}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6,flexWrap:"wrap",gap:8}}>
          <div>
            <div style={{fontSize:16,fontWeight:800,color:"#eef2f8"}}>Recurring Templates</div>
            <div style={{fontSize:11,color:"#3d5166",marginTop:2}}>New items are auto-added to current month</div>
          </div>
          <div style={{display:"flex",gap:8}}>
            <Btn onClick={()=>{onSave(t);onClose();}} variant="solid" color="#06b6d4">Save & Apply</Btn>
        ...
        <div style={{marginBottom:22,marginTop:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{fontSize:11,fontWeight:700,color:"#4ade80",letterSpacing:"0.1em",textTransform:"uppercase"}}>Income</span>
          ...
          {t.income.map(x=>(
            <div key={x.id} style={{display:"flex",gap:8,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}>
              <div style={{flex:"1 1 160px",minWidth:0}}>
                <TxtInput value={x.source} onChange={v=>ui(p=>({income:p.income.map(r=>r.id===x.id?{...r,source:v}:r)}))} placeholder="Source..."/>
              </div>
        ...
        <div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <span style={{fontSize:11,fontWeight:700,color:"#f87171",letterSpacing:"0.1em",textTransform:"uppercase"}}>Expenses</span>
        ...
        {currentMonth && (
          <div style={{marginTop:16,padding:"10px 14px",background:"rgba(6,182,212,0.06)",
            borderRadius:10,border:"1px solid rgba(6,182,212,0.12)",fontSize:11,color:"#4a90d9"}}>
            💡 "Save & Apply" adds any <b>new</b> items (not already in this month) to the current month automatically.
          </div>
        )}
      </div>
    </div>
...
/* ═══════════════════════════════════════════════
   CHARTS
═══════════════════════════════════════════════ */
function TrendChart({ allData, globalDebts }) {
  ...
  const items=keys.map(k=>{
    const[y,m]=k2ym(k);
    const{inc,exp}=calcMonth(allData[k],globalDebts,k);
  ...
  const bw=Math.min(24,(CW/items.length/2)-4),gap=CW/items.length;
  return (
    <div style={{overflowX:"auto"}}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",minWidth:280,height:"auto",display:"block"}}>
        {[0,25,50,75,100].map(p=>{const y=PT+CH*(1-p/100);return <g key={p}>
        ...
        {items.map((item,i)=>{const cx=PL+i*gap+gap/2,ih=(item.inc/mx)*CH,eh=(item.exp/mx)*CH;return <g key={i}>
...
function SavingsChart({ allData, globalDebts }) {
  const keys=Object.keys(allData).sort().slice(-8);
  if(keys.length<2) return null;
  let cum=0;
  const items=keys.map(k=>{const[,m]=k2ym(k);const s=Math.max(0,calcMonth(allData[k],globalDebts,k).sav);cum+=s;return{lbl:SHORT[m],cum};});
  const mx=Math.max(...items.map(i=>i.cum),1);
  const W=560,H=110,PL=48,PB=22,PT=10,CW=W-PL-16,CH=H-PB-PT,step=CW/(items.length-1);
  ...
  return (
    <div style={{overflowX:"auto"}}>
      <svg viewBox={`0 0 ${W} ${H}`} style={{width:"100%",minWidth:280,height:"auto",display:"block"}}>
        ...
        {pts.map((p,i)=><g key={i}><circle cx={p.x} cy={p.y} r={3} fill="#c4b5fd"/>
          <text x={p.x} y={H-5} textAnchor="middle" fontSize={8} fill="#4a90d9">{items[i].lbl}</text></g>)}
      </svg>
    </div>
  );
}
...
function DonutChart({ expenses }) {
  ...
  const R=56,INN=34,CX=75,CY=75;let angle=-Math.PI/2;
  const slices=entries.map(([cat,val])=>{
    const a=(val/total)*2*Math.PI;
    const x1=CX+R*Math.cos(angle),y1=CY+R*Math.sin(angle);angle+=a;
    return{cat,val,x1,y1,x2:CX+R*Math.cos(angle),y2:CY+R*Math.sin(angle),large:a>Math.PI?1:0,color:CAT_CLR[cat]||"#94a3b8"};
  });
  return (
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    saveTimer.current=setTimeout(()=>saveStore(store),400);
  ...
  const md         = store.months[monthKey] || blankMonth(store.templates);
  ...
  useEffect(()=>{
    const m={};
    [...income,...expenses,...globalDebts].forEach(r=>{m[r.id]=true;});
    sv(m);
  },[monthKey]);
  ...
  const handleNextMonth=useCallback(()=>{
    const nm=curM===11?0:curM+1,ny=curM===11?curY+1:curY,nk=mkKey(ny,nm);
    SS(prev=>{
      if(prev.months[nk]) return prev;
      return{...prev,months:{...prev.months,[nk]:blankMonth(prev.templates)}};
    });
    sCY(ny);sCM(nm);
  },[curM,curY]);
  ...
  const handleTemplateSave=useCallback(t=>{
    SS(p=>{
      const c=p.months[monthKey]||{};
      const existingInc=new Set((c.income||[]).map(i=>i.source.toLowerCase().trim()));
      const existingExp=new Set((c.expenses||[]).map(e=>e.name.toLowerCase().trim()));
      const newInc=t.income
        .filter(ti=>!existingInc.has(ti.source.toLowerCase().trim()))
        .map(ti=>({id:uid(),source:ti.source,budgeted:ti.budgeted,actual:0}));
      const newExp=t.expenses
        .filter(te=>!existingExp.has(te.name.toLowerCase().trim()))
        .map(te=>({id:uid(),name:te.name,budgeted:te.budgeted,actual:0,paid:false,category:te.category||"Essential",notes:"",linkedDebtId:null}));
      const updatedMonth={...c,income:[...(c.income||[]),...newInc],expenses:[...(c.expenses||[]),...newExp]};
      return{...p,templates:t,months:{...p.months,[monthKey]:updatedMonth}};
    });
  },[monthKey]);
  ...
  /* ── Totals ── */
  const {inc:totalInc,exp:totalExp,dpaid:loanPaid,sav:mthSav}=calcMonth(md,globalDebts,monthKey);
  const paidExp   = expenses.filter(e=>e.paid).reduce((s,e)=>s+(e.actual||0),0);
  const liveCash  = totalInc-paidExp;
  const netBal    = totalInc-totalExp;
  const totalDebt = globalDebts.reduce((s,d)=>s+(d.totalDebt||0),0);
  const totalPaid = globalDebts.reduce((s,d)=>s+sumPay(d),0);
  ...
  const filteredExp=useMemo(()=>expenses.filter(e=>{
    const mc=expCat==="All"||e.category===expCat;
    const mt=!expSearch||e.name.toLowerCase().includes(expSearch.toLowerCase())||(e.notes||"").toLowerCase().includes(expSearch.toLowerCase());
    return mc&&mt;
  ...
  return (
    <div style={{minHeight:"100vh",
      ...
      fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",color:"#dde4ee",paddingBottom:60}}>
      ...
      <header className="hdr" style={{borderBottom:"1px solid rgba(99,179,237,0.08)",
        background:"rgba(7,9,14,0.93)",backdropFilter:"blur(20px)",position:"sticky",top:0,zIndex:50,
        padding:"13px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#06b6d4,#3b82f6)",
            display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,color:"#fff",fontSize:15,
            flexShrink:0,boxShadow:"0 0 14px rgba(6,182,212,0.35)"}}>S</div>
          <div>
      ...
      <div className="wrap" style={{marginTop:20}}>
        ...
        <div className="ta" style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,gap:8,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            ...
            <Btn onClick={()=>exportCSV(monthKey,md,globalDebts)} size="sm" color="#67e8f9">↓ CSV</Btn>
        ...
        {/* HISTORY STRIP */}
        {allKeys.length>1&&(
          <div style={{marginBottom:14,padding:"11px 14px",background:"rgba(99,102,241,0.04)",
            border:"1px solid rgba(99,102,241,0.1)",borderRadius:12,overflowX:"auto"}}>
            <div style={{fontSize:9,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"#3d5166",marginBottom:8}}>Monthly History</div>
            <div style={{display:"flex",gap:8,minWidth:"max-content"}}>
              {[...allKeys].sort().map(k=>{
                const[ky,km]=k2ym(k);
                const{inc,exp,sav}=calcMonth(store.months[k],globalDebts,k);
                const isA=k===monthKey;
                return <div key={k} onClick={()=>{sCY(ky);sCM(km);}}
                  style={{padding:"9px 12px",borderRadius:10,cursor:"pointer",minWidth:106,flexShrink:0,
                    transition:"all .2s",border:`1px solid ${isA?"rgba(6,182,212,0.4)":"rgba(255,255,255,0.06)"}`,
                    background:isA?"rgba(6,182,212,0.08)":"rgba(255,255,255,0.02)"}}>
                  <div style={{fontSize:10,fontWeight:700,color:isA?"#67e8f9":"#4a90d9",marginBottom:4}}>{MONTHS[km].slice(0,3)} {ky}</div>
        ...
        {/* OVERVIEW METRICS */}
        <div style={{fontSize:9,fontWeight:700,letterSpacing:"0.13em",textTransform:"uppercase",color:"#2a3a4a",marginBottom:9}}>Financial Overview</div>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              ...
              <tbody>
                {income.map(row=>{
                  ...
                  return <FadeRow key={row.id} visible={vis[row.id]??true}>
              ...
              {expCats.map(cat=>(
                <button key={cat} onClick={()=>setExpCat(cat)}
                  style={{padding:"4px 10px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",
                    ...
                    background:expCat===cat?(cat==="All"?"rgba(6,182,212,0.25)":CAT_CLR[cat]+"30"):"rgba(255,255,255,0.04)",
                    color:expCat===cat?(cat==="All"?"#67e8f9":CAT_CLR[cat]||"#94a3b8"):"#3d5166"}}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          <div className="ts">
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              <thead><tr>
                ...
                <th style={{...TH,textAlign:"right",width:110}} className="hm">Budgeted</th>
                <th style={{...TH,textAlign:"right",width:110}}>Actual</th>
                <th style={{...TH,width:90}} className="hm" title="Link to a debt — checking paid will auto-update it">🔗 Debt</th>
                <th style={{...TH,width:60}} className="hm">Notes</th>
                <th style={{...TH,width:36}}/>
              </tr></thead>
              <tbody>
                {filteredExp.map(row=>{
                  const linked=globalDebts.find(d=>d.id===row.linkedDebtId);
                  return <FadeRow key={row.id} visible={vis[row.id]??true}>
                    ...
                    <td style={TD} className={`er ${row.paid?"paid":""}`}>
                      <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                        ...
                        {linked&&<Badge bg="rgba(99,102,241,0.12)" fg="#818cf8">→{linked.name.slice(0,10)}</Badge>}
                      </div>
                    </td>
                ...
                {filteredExp.length===0&&(
                  <tr><td colSpan={8} style={{padding:20,textAlign:"center",color:"#3d5166",fontSize:12}}>
              ...
              <tfoot><tr>
                ...
                <td colSpan={3} style={{borderTop:"1px solid rgba(239,68,68,0.12)"}}/>
              </tr></tfoot>
            </table>
          </div>
        </Card>
        ...
        <Card style={{background:"rgba(99,102,241,0.035)",border:"1px solid rgba(99,102,241,0.1)"}} className="su">
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
            <div>
              <span style={{fontSize:11,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"#818cf8"}}>▸ Debt & Loan Tracker</span>
              <span style={{marginLeft:10,fontSize:11,color:"#3d5166"}}>
                Paid all-time <span style={{color:"#34d399",fontWeight:700}}>{fmt(totalPaid)}</span>
          ...
          {globalDebts.length===0&&(
            <div style={{textAlign:"center",color:"#3d5166",padding:"20px 0",fontSize:12}}>
              No debts yet. Add one above — it will appear on every month until marked complete.
            </div>
          )}
          ...
          {globalDebts.length>0&&(
            <div className="ts" style={{marginBottom:16}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                <thead><tr>
                  <th style={{...TH,width:32,textAlign:"center"}}>✓</th>
                  <th style={TH}>Debt / Loan</th>
                  <th style={{...TH,textAlign:"right",width:110}} className="hm">Total Debt</th>
                  <th style={{...TH,textAlign:"right",width:110}}>This Month</th>
                  <th style={{...TH,textAlign:"right",width:110}} className="hm">All-time Paid</th>
                  <th style={{...TH,textAlign:"right",width:100}}>Remaining</th>
                  <th style={{...TH,width:120}} className="hm">Progress</th>
                ...
                <tbody>
                  {globalDebts.map(row=>{
                    const paid=sumPay(row);
                    const thisMo=(row.payments||{})[monthKey]||0;
                    const rem=Math.max(0,(row.totalDebt||0)-paid);
                    const done=row.completed||rem===0;
                    const p=pct(paid,row.totalDebt||1);
                    const col=done?"#34d399":p>=75?"#fbbf24":"#818cf8";
                    return <FadeRow key={row.id} visible={vis[row.id]??true}>
                      ...
                      <td style={TD} className={`dr ${done?"done":""}`}>
                        <div style={{display:"flex",flexDirection:"column",gap:2}}>
                          ...
                          {done&&<span style={{fontSize:9,color:"#34d399",paddingLeft:8}}>✓ Cleared</span>}
                        </div>
                      </td>
                      ...
                      <td style={{...TD,textAlign:"right"}} className={`dr ${done?"done":""}`}>
                        <NumInput value={thisMo} onChange={v=>setDebtPay(row.id,v)}/>
                      </td>
                      <td style={{...TD,textAlign:"right",color:"#34d399",fontWeight:700}} className={`dr ${done?"done":""} hm`}>{fmt(paid)}</td>
                      ...
                      <td style={TD} className="hm">
                        <div style={{minWidth:90}}>
                          ...
                          <ProgBar value={paid} total={row.totalDebt||0} color={col} height={5}/>
                        </div>
                      </td>
                      <td style={TD} className="hm">
                ...
                <tfoot><tr>
                  ...
                  <td style={{...TD,textAlign:"right",fontWeight:700,color:"#818cf8",borderTop:"1px solid rgba(99,102,241,0.14)"}}>{fmt(loanPaid)}</td>
                  <td style={{...TD,textAlign:"right",fontWeight:700,color:"#34d399",borderTop:"1px solid rgba(99,102,241,0.14)"}} className="hm">{fmt(totalPaid)}</td>
          ...
          {globalDebts.length>0&&(
            <>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:10,marginBottom:16}}>
                {globalDebts.map(row=><DebtCard key={row.id} row={row} visible={vis[row.id]??true}/>)}
              </div>
        ...
        <Card style={{background:"rgba(196,181,253,0.04)",border:"1px solid rgba(196,181,253,0.1)"}} className="su">
          ...
          <div className="cr" style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:14,marginBottom:14}}>
            <MCard label={`${MONTHS[curM]} Savings`} value={fmt(mthSav)} color={mthSav>=0?"#c4b5fd":"#f87171"} sub="Income − Expenses − Loan Payments" danger={mthSav<0}/>
            <MCard label="Total Saved (All Months)" value={fmt(cumSav)} color="#a5b4fc" isHero sub={`Across all ${allKeys.length} month${allKeys.length!==1?"s":""}`}/>
          ...
          {totalInc>0&&(
            <div style={{marginBottom:14,padding:"12px 14px",background:"rgba(196,181,253,0.06)",
              borderRadius:10,border:"1px solid rgba(196,181,253,0.1)"}}>
          ...
          {allKeys.length>=2&&(
            <>
              ...
              <SavingsChart allData={store.months} globalDebts={globalDebts}/>
            </>
          )}
        </Card>

        {/* SUMMARY BAR */}
        <div style={{padding:"14px 18px",background:"rgba(6,182,212,0.03)",border:"1px solid rgba(6,182,212,0.08)",
          ...
          {[
            ...
            {l:"Month Saved",  v:fmt(mthSav),   c:"#c4b5fd"},
      ...
      {modal==="charts"&&(
        <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(0,0,0,0.85)",
          ...
          onClick={()=>sm(null)}>
          <div style={{background:"#0d1117",border:"1px solid rgba(255,255,255,0.1)",borderRadius:18,
            padding:"24px 20px",maxWidth:720,width:"100%",marginTop:40}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{marginBottom:24}}>
              ...
              <TrendChart allData={store.months} globalDebts={globalDebts}/>
            </div>
            <div style={{marginBottom:24}}>
      ...
      {modal==="history"&&<HistoryMgr store={store} onUpdate={s=>SS(s)} onClose={()=>sm(null)}/>}
