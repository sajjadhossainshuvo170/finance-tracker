...
const T = {
  app: { minHeight:"100vh", background:"linear-gradient(160deg,#07090e 0%,#0b0f17 55%,#070a0f 100%)", fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif", color:"#dde4ee", paddingBottom:64 },
  hdr: { borderBottom:"1px solid rgba(99,179,237,0.09)", background:"rgba(7,9,14,0.9)", backdropFilter:"blur(24px)", position:"sticky", top:0, zIndex:50, padding:"16px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" },
  wrap: { maxWidth:1260, margin:"0 auto", padding:"0 22px" },
  card: (x={}) => ({ background:"rgba(255,255,255,0.027)", border:"1px solid rgba(255,255,255,0.065)", borderRadius:16, padding:"20px 22px", transition:"border-color .3s", ...x }),
  th: { padding:"8px 11px", textAlign:"left", color:"#3d5166", fontSize:10, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", borderBottom:"1px solid rgba(255,255,255,0.05)" },
  ...
  badge: (bg,fg) => ({ display:"inline-block", padding:"2px 8px", borderRadius:99, fontSize:10, fontWeight:700, background:bg, color:fg, letterSpacing:"0.05em" }),
  numIn: (f) => ({ background:"rgba(255,255,255,0.048)", border:`1px solid ${f?"rgba(6,182,212,0.55)":"rgba(255,255,255,0.085)"}`, borderRadius:8, padding:"5px 9px", color:"#dde4ee", fontSize:13, width:108, outline:"none", boxShadow:f?"0 0 0 3px rgba(6,182,212,0.1)":"none", transition:"border-color .2s,box-shadow .2s", fontFamily:"inherit" }),
  txtIn: (f) => ({ background:f?"rgba(255,255,255,0.055)":"transparent", border:`1px solid ${f?"rgba(6,182,212,0.38)":"transparent"}`, borderRadius:7, padding:"5px 8px", color:"#c8d3e0", fontSize:13, outline:"none", width:"100%", boxShadow:f?"0 0 0 3px rgba(6,182,212,0.08)":"none", transition:"all .2s", fontFamily:"inherit" }),
  dateIn: (f) => ({ background:"rgba(255,255,255,0.048)", border:`1px solid ${f?"rgba(6,182,212,0.45)":"rgba(255,255,255,0.08)"}`, borderRadius:8, padding:"5px 9px", color:"#8899aa", fontSize:12, width:128, outline:"none", colorScheme:"dark", fontFamily:"inherit", transition:"border-color .2s" }),
...
function NumInput({ value, onChange, width=108 }) {
  ...
  return <input type="number" value={value} onChange={e=>onChange(Number(e.target.value)||0)} onFocus={()=>setF(true)} onBlur={()=>setF(false)} style={{...T.numIn(f),width}} className="hs"/>;
}
function TxtInput({ value, onChange, placeholder="Name…" }) {
  const [f,setF] = useState(false);
  return <input type="text" value={value} onChange={e=>onChange(e.target.value)} onFocus={()=>setF(true)} onBlur={()=>setF(false)} placeholder={placeholder} style={T.txtIn(f)}/>;
}
function DateInput({ value, onChange }) {
  const [f,setF] = useState(false);
  return <input type="date" value={value} onChange={e=>onChange(e.target.value)} onFocus={()=>setF(true)} onBlur={()=>setF(false)} style={T.dateIn(f)}/>;
}
function AddBtn({ onClick, label, color="#06b6d4" }) {
  const [h,setH] = useState(false);
  return <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"6px 13px", borderRadius:8, border:`1px solid ${h?`${color}99`:`${color}44`}`, background:h?`${color}22`:`${color}0e`, color, fontSize:12, fontWeight:700, cursor:"pointer", letterSpacing:"0.04em", transition:"all .2s", fontFamily:"inherit", transform:h?"translateY(-1px)":"none", boxShadow:h?`0 4px 16px ${color}28`:"none" }}><span style={{fontSize:15,lineHeight:1}}>+</span>{label}</button>;
}
function DelBtn({ onClick }) {
  const [h,setH] = useState(false);
  return <button onClick={onClick} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} title="Remove" style={{ width:27, height:27, borderRadius:7, cursor:"pointer", border:`1px solid ${h?"rgba(239,68,68,0.48)":"rgba(239,68,68,0.18)"}`, background:h?"rgba(239,68,68,0.16)":"rgba(239,68,68,0.05)", color:"#f87171", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, transition:"all .2s", fontSize:12, transform:h?"scale(1.1)":"scale(1)" }}>✕</button>;
}
...
function MCard({ label, value, accent=MA.green, isHero, sub, danger }) {
  const [h,setH] = useState(false);
  const heroStyle = { background:"linear-gradient(135deg,rgba(6,182,212,0.13),rgba(59,130,246,0.09))", border:"1px solid rgba(6,182,212,0.28)", borderRadius:16, padding:"20px 22px", flex:1, minWidth:175, position:"relative", overflow:"hidden", cursor:"default", boxShadow:h?"0 0 48px rgba(6,182,212,0.2),0 10px 36px rgba(0,0,0,0.35)":"0 0 22px rgba(6,182,212,0.09)", transform:h?"translateY(-4px)":"translateY(0)", transition:"all .25s" };
  const normStyle = { background:`linear-gradient(135deg,${accent.g1},${accent.g2})`, border:`1px solid ${accent.bd}`, borderRadius:16, padding:"20px 22px", flex:1, minWidth:175, cursor:"default", transform:h?"translateY(-3px)":"translateY(0)", transition:"all .24s" };
  return (
    <div style={isHero?heroStyle:normStyle} onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)}>
      <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.11em", textTransform:"uppercase", color:isHero?"#67e8f9":"#7a8fa8", marginBottom:8 }}>{label}</div>
      <div style={{ fontSize:isHero?27:22, fontWeight:800, color:danger?"#f87171":isHero?"#e0f7fa":(accent?.val||"#f1f5f9"), letterSpacing:"-0.025em", lineHeight:1 }}>{value}</div>
  ...
  return (
    <div>
      ...
      <div style={{...T.track,width:"100%"}}>
        <div style={{ height:"100%", borderRadius:99, background:grad, boxShadow:`0 0 8px ${glow}`, width:`${p}%`, transition:"width .6s cubic-bezier(.4,0,.2,1)" }}/>
      </div>
    </div>
  );
}
function DebtCard({ row, visible }) {
  ...
  const [h,setH] = useState(false);
  return (
    <div onMouseEnter={()=>setH(true)} onMouseLeave={()=>setH(false)} style={{ background:done?"rgba(5,150,105,0.07)":"rgba(255,255,255,0.025)", border:`1px solid ${done?"rgba(52,211,153,0.2)":h?"rgba(129,140,248,0.25)":"rgba(255,255,255,0.065)"}`, borderRadius:14, padding:"16px 18px", opacity:visible?1:0, transform:visible?"translateY(0)":"translateY(14px)", transition:"opacity .35s ease,transform .35s ease,border-color .25s", boxShadow:h&&!done?"0 4px 24px rgba(129,140,248,0.1)":"none" }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ fontSize:13, fontWeight:700, color:done?"#6ee7b7":"#c8d3e0", textDecoration:done?"line-through":"none", marginBottom:3 }}>{row.name}</div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:7, marginLeft:8 }}>
          {done?<span style={T.badge("rgba(5,150,105,0.18)","#34d399")}>COMPLETED</span>:<span style={T.badge("rgba(99,102,241,0.16)","#818cf8")}>ACTIVE</span>}
        </div>
      </div>
...
function MonthNav({ year, month, onChange, allKeys }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, position:"relative" }}>
      <button onClick={()=>go(-1)} style={{ width:30, height:30, borderRadius:8, border:"1px solid rgba(6,182,212,0.3)", background:"rgba(6,182,212,0.08)", color:"#67e8f9", cursor:"pointer", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>‹</button>
      <button onClick={()=>setDdOpen(p=>!p)} style={{ padding:"6px 14px", borderRadius:9, border:"1px solid rgba(6,182,212,0.3)", background:"rgba(6,182,212,0.08)", color:"#67e8f9", cursor:"pointer", fontWeight:700, fontSize:13, fontFamily:"inherit", minWidth:140, textAlign:"center" }}>
        {MONTHS[month]} {year} {ddOpen?"▲":"▼"}
      </button>
      <button onClick={()=>go(1)} style={{ width:30, height:30, borderRadius:8, border:"1px solid rgba(6,182,212,0.3)", background:"rgba(6,182,212,0.08)", color:"#67e8f9", cursor:"pointer", fontSize:16, display:"flex", alignItems:"center", justifyContent:"center" }}>›</button>

      {ddOpen && (
        <div style={{ position:"absolute", top:38, left:32, zIndex:100, background:"#0d1117", border:"1px solid rgba(6,182,212,0.25)", borderRadius:12, padding:8, minWidth:180, boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
          {sorted.length === 0 && <div style={{ color:"#3d5166", fontSize:12, padding:"6px 10px" }}>No saved months</div>}
          {sorted.map(k => {
            const [ky, km] = k.split("-");
            ...
            return (
              <div key={k} onClick={()=>{ onChange(parseInt(ky), parseInt(km)-1); setDdOpen(false); }}
                style={{ padding:"7px 12px", borderRadius:8, cursor:"pointer", fontSize:13, fontWeight:isActive?700:400, color:isActive?"#67e8f9":"#c8d3e0", background:isActive?"rgba(6,182,212,0.12)":"transparent", marginBottom:2, transition:"background .15s" }}>
                {label}{isActive?" ✓":""}
              </div>
            );
          })}
          <div style={{ borderTop:"1px solid rgba(255,255,255,0.05)", marginTop:6, paddingTop:6 }}>
            <div style={{ padding:"4px 12px", fontSize:10, color:"#3d5166", fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase" }}>Jump to year</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:4, padding:"4px 8px" }}>
              {[...new Set(sorted.map(k=>k.split("-")[0]))].map(y=>(
                <button key={y} onClick={()=>{ onChange(parseInt(y), month); setDdOpen(false); }}
                  style={{ padding:"3px 10px", borderRadius:6, border:"1px solid rgba(6,182,212,0.2)", background:"rgba(6,182,212,0.07)", color:"#67e8f9", cursor:"pointer", fontSize:11, fontFamily:"inherit" }}>{y}</button>
              ))}
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  // All months data stored as { "2025-06": { income, expenses, debts }, ... }
  const [allData, setAllData] = useState(() => {
    const saved = loadLS("sft_all_months_v1", null);
  ...
  // Get or create current month data
  const monthData = allData[monthKey] || freshMonth();
  const income    = monthData.income   || [];
  const expenses  = monthData.expenses || [];
  const debts     = monthData.debts    || [];

  const isReadOnly = false; // all months are editable

  // Persist
  useEffect(()=>{ saveLS("sft_all_months_v1", allData); }, [allData]);

  // Rebuild vis when month changes
  useEffect(()=>{
    const m={};
  ...
  function handleNewMonth() {
    ...
    if(!allData[nextKey]) {
      setAllData(prev => ({ ...prev, [nextKey]: freshMonth() }));
    }
    setCurYear(nextY);
    setCurMonth(nextM);
  }
  ...
  // Income
  const addI  = ()=>{ const id=uid(); updateMonth({income:[...income,{id,source:"New Source",budgeted:0,actual:0}]}); showRow(id); };
  const delI  = useCallback(id=>hideRow(id,id=>updateMonth({income:income.filter(r=>r.id!==id)})),[income,monthKey]); // eslint-disable-line
  const setIF = useCallback((id,k,v)=>updateMonth({income:income.map(r=>r.id===id?{...r,[k]:v}:r)}),[income,monthKey]); // eslint-disable-line

  // Expenses
  const addE  = ()=>{ const id=uid(); updateMonth({expenses:[...expenses,{id,name:"New Expense",budgeted:0,actual:0,paid:false}]}); showRow(id); };
  const delE  = useCallback(id=>hideRow(id,id=>updateMonth({expenses:expenses.filter(r=>r.id!==id)})),[expenses,monthKey]); // eslint-disable-line
  const setEF = useCallback((id,k,v)=>updateMonth({expenses:expenses.map(r=>r.id===id?{...r,[k]:v}:r)}),[expenses,monthKey]); // eslint-disable-line

  // Debts
  const addD  = ()=>{ const id=uid(); updateMonth({debts:[...debts,{id,name:"New Debt",totalDebt:0,paid:0,dueDate:"",completed:false}]}); showRow(id); };
  const delD  = useCallback(id=>hideRow(id,id=>updateMonth({debts:debts.filter(r=>r.id!==id)})),[debts,monthKey]); // eslint-disable-line
  const setDF = useCallback((id,k,v)=>updateMonth({debts:debts.map(r=>r.id===id?{...r,[k]:v}:r)}),[debts,monthKey]); // eslint-disable-line
    ...
    <div style={T.app}>
      ...
      <header style={T.hdr}>
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          ...
          {isCurrentMonth && <span style={T.badge("rgba(34,197,94,0.15)","#4ade80")}>● CURRENT</span>}
          {!isCurrentMonth && <span style={T.badge("rgba(251,191,36,0.12)","#fbbf24")}>HISTORY</span>}
        </div>
      </header>
      ...
      <div style={{...T.wrap,marginTop:26}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18,flexWrap:"wrap",gap:10}}>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            <span style={T.badge("rgba(6,182,212,0.11)","#67e8f9")}>{MONTHS[curMonth]} {curYear}</span>
            ...
            <span style={T.badge("rgba(99,102,241,0.1)","#818cf8")}>{allKeys.length} Month{allKeys.length!==1?"s":""} Saved</span>
          </div>
          {isCurrentMonth && (
            <button onClick={handleNewMonth} style={{ padding:"7px 16px", borderRadius:9, border:"1px solid rgba(6,182,212,0.4)", background:"rgba(6,182,212,0.1)", color:"#67e8f9", cursor:"pointer", fontWeight:700, fontSize:12, fontFamily:"inherit", letterSpacing:"0.04em", transition:"all .2s" }}>
              + Start Next Month →
            </button>
          )}
        </div>
        ...
        {allKeys.length > 1 && (
          <div style={{marginBottom:20,padding:"12px 16px",background:"rgba(99,102,241,0.04)",border:"1px solid rgba(99,102,241,0.1)",borderRadius:12,overflowX:"auto"}}>
            <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"#3d5166",marginBottom:10}}>── Monthly History</div>
            <div style={{display:"flex",gap:8,flexWrap:"nowrap",minWidth:"max-content"}}>
              {[...allKeys].sort().map(k=>{
                ...
                return (
                  <div key={k} onClick={()=>{setCurYear(parseInt(ky));setCurMonth(parseInt(km)-1);}}
                    style={{ padding:"10px 14px", borderRadius:10, border:`1px solid ${isActive?"rgba(6,182,212,0.4)":"rgba(255,255,255,0.06)"}`, background:isActive?"rgba(6,182,212,0.08)":"rgba(255,255,255,0.02)", cursor:"pointer", minWidth:120, transition:"all .2s", flexShrink:0 }}>
        ...
        {/* FINANCIAL OVERVIEW */}
        <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.14em",textTransform:"uppercase",color:"#2a3a4a",marginBottom:10}}>── Financial Overview</div>
        <div style={{display:"flex",gap:13,flexWrap:"wrap",marginBottom:10}}>
          <MCard label="Total Income"    value={fmt(totalIncome)}   accent={MA.green}  sub={`${income.length} streams · budget ${fmt(budgetedIncome)}`}/>
          <MCard label="Total Expenses"  value={fmt(totalExpenses)} accent={MA.red}    sub={`${expenses.length} items · budget ${fmt(budgetedExp)}`}/>
          <MCard label="Live Cash in Hand" value={fmt(liveCash)} isHero sub={`After ${fmt(paidExpenses)} paid`}/>
          <MCard label="Net Savings" value={fmt(netSavings)} accent={MA.purple} sub={netSavings>=0?"Positive balance ↑":"Over budget ↓"} danger={netSavings<0}/>
        </div>

        {/* DEBT OVERVIEW */}
        <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.14em",textTransform:"uppercase",color:"#2a3a4a",margin:"22px 0 10px"}}>── Debt Overview</div>
        <div style={{display:"flex",gap:13,flexWrap:"wrap",marginBottom:28}}>
          <MCard label="Total Debt Owed" value={fmt(totalDebt)}    accent={MA.red}    sub={`${debts.length} obligation${debts.length!==1?"s":""}`}/>
          <MCard label="Total Paid"       value={fmt(totalPaid)}    accent={MA.green}  sub={`${debtClearPct}% cleared`}/>
          <MCard label="Remaining Debt"   value={fmt(remainDebt)}   accent={MA.amber}  sub={`${100-debtClearPct}% outstanding`} danger={remainDebt>0}/>
          <MCard label="Debt Clearance"   value={`${debtClearPct}%`} accent={MA.orange} sub={`${fmt(totalPaid)} of ${fmt(totalDebt)}`}/>
        </div>
                ...
                {income.map(row=>{
                  ...
                  return (
                    <FadeRow key={row.id} visible={vis[row.id]??false}>
                      ...
                      <td style={{...T.td,textAlign:"right"}}><span style={{fontWeight:700,fontSize:12,color:v>=0?"#4ade80":"#f87171"}}>{v>=0?"+":""}{fmt(v)}</span></td>
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                ...
                {expenses.map(row=>(
                  <FadeRow key={row.id} visible={vis[row.id]??false}>
                    <td style={{...T.td,textAlign:"center"}} className={`er ${row.paid?"paid":""}`}>
                      <input type="checkbox" checked={row.paid} onChange={()=>setEF(row.id,"paid",!row.paid)} style={{width:15,height:15,cursor:"pointer",accentColor:"#06b6d4"}}/>
                    </td>
                    ...
                    <td style={{...T.td,textAlign:"right"}} className={`er ${row.paid?"paid":""}`}><NumInput value={row.budgeted} onChange={v=>setEF(row.id,"budgeted",v)}/></td>
                    <td style={{...T.td,textAlign:"right"}} className={`er ${row.paid?"paid":""}`}><NumInput value={row.actual}   onChange={v=>setEF(row.id,"actual",v)}/></td>
              ...
              <tfoot><tr style={{background:"rgba(239,68,68,0.03)"}}>
                ...
                <td style={{...T.td,textAlign:"right",color:"#3d5166",borderTop:"1px solid rgba(239,68,68,0.11)"}}>{fmt(budgetedExp)}</td>
        ...
        <div style={{...T.card({background:"rgba(99,102,241,0.035)",border:"1px solid rgba(99,102,241,0.1)"}),marginBottom:20}} className="su">
            ...
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
              ...
              <tbody>
                {debts.map(row=>{
                  ...
                  const barGrad=done?"linear-gradient(90deg,#059669,#34d399)":p>=75?"linear-gradient(90deg,#b45309,#fbbf24)":"linear-gradient(90deg,#4338ca,#818cf8)";
                  return (
                    <FadeRow key={row.id} visible={vis[row.id]??false}>
                      <td style={{...T.td,textAlign:"center"}} className={`dr ${done?"done":""}`}>
                        <input type="checkbox" checked={row.completed} onChange={()=>setDF(row.id,"completed",!row.completed)} style={{width:15,height:15,cursor:"pointer",accentColor:"#34d399"}}/>
                      </td>
                      <td style={T.td} className={`dr ${done?"done":""}`}><TxtInput value={row.name} onChange={v=>setDF(row.id,"name",v)} placeholder="Debt name…"/></td>
                      <td style={{...T.td,textAlign:"right"}} className={`dr ${done?"done":""}`}><NumInput value={row.totalDebt} onChange={v=>setDF(row.id,"totalDebt",v)}/></td>
                      <td style={{...T.td,textAlign:"right"}} className={`dr ${done?"done":""}`}><NumInput value={row.paid} onChange={v=>setDF(row.id,"paid",clamp(v,0,row.totalDebt||999999))}/></td>
                      <td style={{...T.td,textAlign:"right",fontWeight:700,color:done?"#34d399":remaining>0?"#f87171":"#4ade80"}}>{fmt(remaining)}</td>
                      <td style={T.td}>
                        <div style={{minWidth:90}}>
                          ...
                          <div style={{...T.track,width:"100%"}}>
                            <div style={{height:"100%",borderRadius:99,background:barGrad,boxShadow:`0 0 7px ${barColor}66`,width:`${p}%`,transition:"width .55s cubic-bezier(.4,0,.2,1)"}}/>
                          </div>
                        </div>
                      </td>
                      <td style={T.td}><DateInput value={row.dueDate||""} onChange={v=>setDF(row.id,"dueDate",v)}/></td>
          ...
          {debts.length>0&&(
            <>
              <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"#3d5166",marginBottom:12,paddingTop:4,borderTop:"1px solid rgba(255,255,255,0.04)"}}>Individual Debt Progress</div>
          ...
          <div style={{marginTop:20,padding:"16px 18px",background:"rgba(99,102,241,0.07)",borderRadius:12,border:"1px solid rgba(99,102,241,0.1)"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
              <div>
                ...
                <div style={{fontSize:22,fontWeight:800,color:"#a5b4fc",letterSpacing:"-0.02em"}}>{fmt(totalPaid)} <span style={{fontSize:14,color:"#3d5166",fontWeight:400}}>/ {fmt(totalDebt)}</span></div>
              </div>
            ...
            <div style={{height:8,background:"rgba(255,255,255,0.055)",borderRadius:99,overflow:"hidden"}}>
              <div style={{height:"100%",borderRadius:99,background:"linear-gradient(90deg,#4338ca,#818cf8,#a5b4fc)",boxShadow:"0 0 10px rgba(129,140,248,0.5)",width:`${debtClearPct}%`,transition:"width .7s cubic-bezier(.4,0,.2,1)"}}/>
            </div>
            <div style={{display:"flex",justifyContent:"space-between",marginTop:7,fontSize:10,color:"#2a3a4a"}}>
              <span>৳0</span><span style={{color:"#818cf8"}}>{fmt(totalDebt)} Target</span>
            </div>
          </div>
        </div>
        ...
        <div style={{padding:"15px 20px",background:"rgba(6,182,212,0.03)",border:"1px solid rgba(6,182,212,0.09)",borderRadius:12,display:"flex",gap:24,flexWrap:"wrap",alignItems:"center"}}>
          <span style={{fontSize:9,fontWeight:700,letterSpacing:"0.13em",textTransform:"uppercase",color:"#2a3a4a",flexShrink:0}}>Master Summary — {MONTHS[curMonth]} {curYear}</span>
          {[
